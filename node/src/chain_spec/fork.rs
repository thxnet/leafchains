...
/// - `ParachainInfo.ParachainId`, `System.Account`, `:code`, `:heappages` are
///   all preserved (not in any drop table).
...
/// A collator entry for a fork genesis: `(account_id, initial_balance,
/// aura_key)`.
///
...
pub fn assemble_general_fork_genesis(
    ...
    general_runtime::GenesisConfig {
        ...
        collator_selection: general_runtime::CollatorSelectionConfig {
            invulnerables: collators.iter().map(|(acc, ..)| acc.clone()).collect(),
        ...
        session: general_runtime::SessionConfig {
            keys: collators
                .into_iter()
                .map(|(acc, _, aura)| (acc.clone(), acc, general_runtime::SessionKeys { aura }))
                .collect(),
        },
...
mod tests {
    ...
    fn pallet_prefix(name: &[u8]) -> Vec<u8> { twox_128(name).to_vec() }
    ...
    fn filter_drops_parachain_system_validation_data() {
        ...
        let input = storage_with_keys(&[key.clone()]);
        let output = filter_forked_storage(input);
        assert!(!output.top.contains_key(&key), "ParachainSystem.ValidationData must be dropped");
    }
    ...
    fn filter_preserves_parachain_info_parachain_id() {
        ...
        assert!(
            output.top.contains_key(&key),
            "ParachainInfo.ParachainId must be preserved — dropping it would break parachain \
             registration"
        );
    }
    ...
    fn assemble_general_fork_genesis_populates_all_12_fields_without_panic() {
        ...
        assert_eq!(
            genesis.parachain_info.parachain_id, para_id,
    ...
    fn assemble_general_fork_genesis_extra_endowed_is_additive() {
        ...
        let alice_entry = genesis.balances.balances.iter().find(|(acc, _)| acc == &alice_acc);
        ...
        let dave_entry = genesis.balances.balances.iter().find(|(acc, _)| acc == &dave);
    ...
    fn assemble_general_fork_genesis_root_key_reflected_in_sudo() {
        ...
        assert_eq!(genesis_no_sudo.sudo.key, None, "sudo.key must be None when root_key is None");
    }
    ...
    fn drop_lists_are_deduplicated_and_sorted() {
        ...
        // The Ord impl on (&[u8], &[u8]) is lexicographic, which is exactly what we
        // want.
        assert!(
            LEAFCHAIN_DROP_ITEMS.windows(2).all(|w| w[0] < w[1]),
            "LEAFCHAIN_DROP_ITEMS must be strictly sorted by (pallet, item) — no duplicates, no \
             mis-ordering"
        );
    ...
    fn filter_and_assemble_merge_produces_no_collision() {
        ...
        let storage =
            sp_core::storage::Storage { top: raw_top, children_default: Default::default() };
        ...
        let assembled =
            assemble_general_fork_genesis(&wasm, collators, Some(alice), para_id, vec![]);

        // Assert assembled.parachain_info.parachain_id == ParaId::from(7).
        assert_eq!(
            assembled.parachain_info.parachain_id, para_id,
    ...
    fn filter_drops_all_leafchain_drop_items() {
        let keys: Vec<Vec<u8>> =
            LEAFCHAIN_DROP_ITEMS.iter().map(|(pallet, item)| item_prefix(pallet, item)).collect();
