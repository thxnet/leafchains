...
impl<T: Config<I>, I: 'static> Inspect<<T as SystemConfig>::AccountId> for Pallet<T, I> {
    ...
    /// Returns the attribute value of `item` of `collection` corresponding to
    /// `key`.
    ///
    ...
    /// Returns the custom attribute value of `item` of `collection`
    ...
    /// Returns the system attribute value of `item` of `collection`
    /// corresponding to `key`.
    ///
    /// By default this is `None`; no attributes are defined.
    fn system_attribute(
...
impl<T: Config<I>, I: 'static> Create<<T as SystemConfig>::AccountId, CollectionConfigFor<T, I>>
    for Pallet<T, I>
{
    /// Create a `collection` of nonfungible items to be owned by `who` and
    /// managed by `admin`.
...
impl<T: Config<I>, I: 'static> InspectEnumerable<T::AccountId> for Pallet<T, I> {
    ...
    fn collections() -> Self::CollectionsIterator { Collection::<T, I>::iter_keys() }
    ...
    ///
    /// NOTE: iterating this list invokes a storage read per item.
    fn owned(who: &T::AccountId) -> Self::OwnedIterator { Account::<T, I>::iter_key_prefix((who,)) }
