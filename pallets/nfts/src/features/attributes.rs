...
impl<T: Config<I>, I: 'static> Pallet<T, I> {
    pub(crate) fn do_set_attribute(
        ...
        // for the `CollectionOwner` namespace we need to check if the collection/item
        // is not locked
        match namespace {
        ...
        // NOTE: in the CollectionOwner namespace if the depositor is `None` that means
        // the deposit was paid by the collection's owner.
        let old_depositor =
        ...
        // NOTE: when we transfer an item, we don't move attributes in the ItemOwner
        // namespace. When the new owner updates the same attribute, we will
        // update the depositor record and return the deposit to the previous
        // owner.
    ...
    pub(crate) fn do_set_attributes_pre_signed(
        ...
        // Only the CollectionOwner and Account() namespaces could be updated in this
        // way. For the Account() namespace we check and set the approval if it
        // wasn't set before.
        match &namespace {
    ...
    pub(crate) fn do_clear_attribute(
        ...
        if let Some(check_origin) = &maybe_check_origin {
            // validate the provided namespace when it's not a root call and the caller is
            // not the same as the `deposit.account` (e.g. the deposit was paid
            ...
            // can't clear `CollectionOwner` type attributes if the collection/item is
            // locked
            match namespace {
                AttributeNamespace::CollectionOwner => match maybe_item {
                    None => {
