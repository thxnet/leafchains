...
fn account(id: u8) -> AccountIdOf<Test> { [id; 32].into() }
...
fn default_item_config() -> ItemConfig { ItemConfig { settings: ItemSettings::all_enabled() } }
...
fn transfer_owner_should_work() {
    new_test_ext().execute_with(|| {
        ...
        // 2's acceptance from before is reset when it became an owner, so it cannot be
        // transferred without a fresh acceptance.
        assert_noop!(
...
fn set_item_owner_attributes_should_work() {
    new_test_ext().execute_with(|| {
        ...
        // validate the attribute are still here & the deposit belongs to the previous
        // owner
        assert_eq!(
            attributes(0),
            vec![
...
fn validate_deposit_required_setting() {
    new_test_ext().execute_with(|| {
        ...
        // with the disabled DepositRequired setting, only the collection's owner can
        // set the attributes for free.
        assert_ok!(Nfts::force_create(
            RuntimeOrigin::root(),
            account(1),
            default_collection_config()
...
fn approval_lifecycle_works() {
    new_test_ext().execute_with(|| {
        ...
        // ensure we can't buy an item when the collection has a NonTransferableItems
        // flag
        let collection_id = 1;
        assert_ok!(Nfts::force_create(
            RuntimeOrigin::root(),
...
fn approval_deadline_works() {
    new_test_ext().execute_with(|| {
        ...
        // make a new approval with a deadline after 4 blocks, so it will expire after
        // the 5th block.
        assert_ok!(Nfts::approve_transfer(
            RuntimeOrigin::signed(account(4)),
            0,
...
fn mint_settings_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            ItemConfigOf::<Test>::get(collection_id, item_id).unwrap().settings.get_disabled(),
...
fn pre_signed_attributes_should_work() {
    new_test_ext().execute_with(|| {
        ...
        // validate the deposit gets returned on attribute update from collection's
        // owner
        assert_ok!(Nfts::set_attribute(
            RuntimeOrigin::signed(user_1.clone()),
            collection_id,
        ...
        // can't update the CollectionOwner namespace if the signer is not an owner of
        // that collection
        let pre_signed_data = PreSignedAttributes {
            collection: 0,
            item: 0,
