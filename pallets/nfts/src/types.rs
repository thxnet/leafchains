...
pub struct CollectionDetails<AccountId, DepositBalance> {
    ...
    /// The total balance deposited by the owner for all the storage data
    /// associated with this collection. Used by `destroy`.
...
pub struct DestroyWitness {
    /// The total number of items in this collection that have outstanding item
    /// metadata.
    #[codec(compact)]
...
pub struct ItemDetails<AccountId, Deposit, Approvals> {
    ...
    /// The amount held in the pallet's default account for this item. Free-hold
    /// items will have this as zero.
...
pub struct CollectionMetadata<Deposit, StringLimit: Get<u32>> {
    ...
    pub(super) deposit: Deposit,
    /// General information concerning this collection. Limited in length by
    /// `StringLimit`. This will generally be either a JSON dump or the hash
    /// of some JSON which can be found on a hash-addressable global
    /// publication system such as IPFS.
...
pub struct ItemMetadata<Deposit, StringLimit: Get<u32>> {
    ...
    /// This pays for the data stored in this struct.
    pub(super) deposit: Deposit,
    /// General information concerning this item. Limited in length by
    /// `StringLimit`. This will generally be either a JSON dump or the hash
    /// of some JSON which can be found on a hash-addressable global
    /// publication system such as IPFS.
    pub(super) data: BoundedVec<u8, StringLimit>,
...
pub enum CollectionSetting {
    ...
    UnlockedMaxSupply,
    /// When this isn't set then the deposit is required to hold the items of
    /// this collection.
    DepositRequired,
}
...
impl CollectionSettings {
    pub fn all_enabled() -> Self { Self(BitFlags::EMPTY) }

    pub fn get_disabled(&self) -> BitFlags<CollectionSetting> { self.0 }

    pub fn is_disabled(&self, setting: CollectionSetting) -> bool { self.0.contains(setting) }

    pub fn from_disabled(settings: BitFlags<CollectionSetting>) -> Self { Self(settings) }
}

impl_codec_bitflags!(CollectionSettings, u64, CollectionSetting);

/// Mint type. Can the NFT be create by anyone, or only the creator of the
/// collection, or only by wallets that already hold an NFT from a certain
/// collection? The ownership of a privately minted NFT is still publicly
/// visible.
...
impl ItemSettings {
    ...
    pub fn get_disabled(&self) -> BitFlags<ItemSetting> { self.0 }

    pub fn is_disabled(&self, setting: ItemSetting) -> bool { self.0.contains(setting) }

    pub fn from_disabled(settings: BitFlags<ItemSetting>) -> Self { Self(settings) }
}
...
impl ItemConfig {
    ...
    pub fn has_disabled_settings(&self) -> bool { !self.settings.get_disabled().is_empty() }

    pub fn enable_setting(&mut self, setting: ItemSetting) { self.settings.0.remove(setting); }

    pub fn disable_setting(&mut self, setting: ItemSetting) { self.settings.0.insert(setting); }
}
...
impl PalletFeatures {
    ...
    pub fn from_disabled(features: BitFlags<PalletFeature>) -> Self { Self(features) }

    pub fn is_enabled(&self, feature: PalletFeature) -> bool { !self.0.contains(feature) }
}
...
impl CollectionRoles {
    pub fn none() -> Self { Self(BitFlags::EMPTY) }

    pub fn has_role(&self, role: CollectionRole) -> bool { self.0.contains(role) }

    pub fn add_role(&mut self, role: CollectionRole) { self.0.insert(role); }
